...
public class Whisper_Server {
	// 대화명, 클라이언트의 Socket을 저장하기 위한 Map변수 선언
	Map<String, Socket> clients;
	
	// 생성자
	public Whisper_Server() {
	...
	public void sendMessage(String msg, String from) {
		...
		if(msg.substring(0,2).equals("/w")) {  // /w 형태로 보내는 경우 : 귓속말보내기로  판단
			
			for(String str : msg.split(" ", 3)){ // 3개의 배열로 나눔. ["/w", "상대방닉네임", "msg"]
				arr.add(str);
			}
			String to = arr.get(1);  // 보내는이 
			
			DataOutputStream dos;
			try {
				dos = new DataOutputStream(clients.get(to).getOutputStream());
				dos.writeUTF("소곤소곤 from["+from+"]"+ msg); //  귓속말 메시지 보내기
	...
	public static void main(String[] args) {
		new Whisper_Server().startServer();
	}
}
